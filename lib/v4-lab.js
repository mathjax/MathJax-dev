...
/**
 * @fileoverview  The code that handles the v4-lab browser document
 *
...
import {fontName} from '@mathjax/src/js/output/chtml/DefaultFont.js';

import {source} from '@mathjax/src/components/src/source.js';

/**
 * The names of all the TeX packages
 */
const texPackages = Object.keys(source)
 ...
 */
function MathDocumentMixin(documentClass) {
...
/**
 * An extension of the HTML adaptor that handles the web worker properly for the lab
 */
class labAdaptor extends HTMLAdaptor {
  replace(nnode, onode) {
    return this.parent(onode)?.replaceChild(nnode, onode);
  }

  async createWorker(listener, options) {
...
window.MathJax = {
  loader: {
    load: [
      ...
      '[mml]/mml3',
      'ui/menu',
//    'output/chtml',    // loaded in Startup() below
//    ...texPackages     // loaded in Startup() below
    ],
    paths: {
      sre: './node_modules/@mathjax/src/mjs/a11y/sre',
      fonts: '[mathjax]/../../../@mathjax',
    ...
    json: (file) => import(
      file
        .replace('/bundle/', '/ts/')
        .replace('/tex/extensions/', '/tex/')
        .replace('/mathml/extensions/', '/mathml/')
        .replace('/input/mml/', '/input/mathml/'),
      {with: {type: 'json'}}
    ),
  },
  output: {
  ...
  options: {
    ...
    renderActions: {
      ...
      outputMML: [
        STATE.CONVERT + 10,
        (doc) => Lab.outputMML(Array.from(doc.math)[0]),
        (math, doc) => Lab.outputMML(math)
      ],
    }
  },
  startup: {
  ...
  svg: {},
  chtml: {
    fontURL: `./node_modules/@mathjax/${fontName}-font/chtml/woff2`
  }
};
...
const Lab = window.Lab = {
  ...
  adaptor: new labAdaptor(window),              // for creating new nodes more easily
  ...
  menu: {
    ...
    regions: false,                             // true to keep regions when expression loses focus
  },
  packages: {
  ...
  keepOptions: [
    ...
    ['output', {
      ...
      font: ['asana', 'bonum', 'dejavu', 'fira', 'modern', 'newcm', 'pagella', 'schola', 'stix2', 'termes', 'tex'],
  ...
  indexEncoding: '0123456789ABCDEFGHIJKLMNOPQRSTUVQXYZ',
  ...
  /**
   * Perform the typesetting of the math
   */
  Typeset() {
  ...
  findMath() {
    if (this.doc.processed.isSet('findMath')) {
      return;
    }
    //
    ...
    this.doc.processed.set('findMath');
  },
    ...
    for (const item of data) {
      ...
      for (const [key, option] of Object.entries(this[name])) {
        ...
        case 'boolean':
          ...
          break;
        case 'string':
          params.push(this.indexEncoding.charAt((values[key] || []).indexOf(option)));
          break;
        case 'object':
    ...
    for (const item of data) {
      ...
      for (const key of Object.keys(options)) {
        ...
        case 'boolean':
          ...
          break;
        case 'string':
          options[key] = (values[key][this.indexEncoding.indexOf(c)] || '');
          break;
        case 'object':
  ...
  createCheckbox(type, key, checked, div) {
    ...
    checkbox.onchange = () => {this.newPackages(); this.Typeset()};
   ...
   *   for the input jax of the current document.
   *
   */
  newPackages() {
  ...
  newTexInput() {
    ...
    this.doc.inputJax[0] = this.doc.inputJax.tex = jax;
  },
  ...
  setFont(font) {
    ...
    MathJax.config.loader.paths[font] = `[fonts]/${font}-font`;
    const promises = [];
    for (const jax of ['chtml', 'svg']) {
      if (MathJax._.output[jax]) {
        promises.push(this.getOutputPromise(font, jax));
      }
    };
  ...
  /**
   * Get a promise for loading a renderer
   */
  getOutputPromise(font, jax) {
    return MathJax.loader.load(`[${font}]/${jax}`).then(() => {
      if (jax === 'chtml') {
        MathJax.config.chtml.fontURL = `./node_modules/@mathjax/${font}-font/chtml/woff2`;
      }
      ...
      if (this.render.jax.toLowerCase() === jax) {
        this.newOutput();
        this.Typeset();
      }
    });
  ...
  /**
   * Sets whether or not to keep the explorer regions open when the expression loses
   * focus (for debugging purposes).
   *
   * @param {boolean} checked   Whether to keep regions open or not
   */
  setRegions(checked) {
    this.menu.regions = checked;
    Lab.doc.options.keepRegions = checked;
    if (!checked) {
      const explorers = Lab.mathItem.explorers;
      for (const name of explorers.attached) {
        if (name === 'speech') {
          explorers.speech.setCurrent(null);
        }
        explorers[name].Stop();
      }
    }
  ...
  setCollapse(checked) {
    ...
    if (Array.from(this.doc.math).length === 0) {
  ...
  initMenu() {
    ...
    this.menuVariable('brailleSpeech').registerCallback(() => {
      const checked = this.doc.menu.settings.speech;
      this.menu.speech = checked;
      document.getElementById('speech').checked = checked;
    });
    ...
    this.menuVariable('assistiveMml').registerCallback(() => {
      const checked = this.doc.menu.settings.assistiveMml;
      if (checked) {
        for (const key of ['speech', 'braille']) {
          this.menu[key] = false,
          document.getElementById(key).checked = false;
        }
      }
    ...
    this.menuVariable('renderer').registerCallback(() => {
      const jax = this.doc.menu.settings.renderer;
      if (jax === this.doc.outputJax.name && jax === this.render.jax) return;
      this.ready = false;
      document.getElementById('jax').value = this.render.jax = jax;
      MathJax.config.startup.output = jax.toLowerCase();
      document.getElementById('fontCache').disabled = (jax === 'CHTML');
      document.getElementById('adaptiveCSS').disabled = (jax === 'SVG');
      document.getElementById('matchFontHeight').disabled = (jax === 'SVG');
      const promises = this.doc.menu.constructor.loadingPromises;
      const promise = promises.get('output/' + jax.toLowerCase()) || Promise.resolve();
      promise.then(() => {
        this.newOutput();
        this.ready = true;
        this.Typeset();
   ...
   */
  menuHandler(handler) {
    handler.documentClass = MathDocumentMixin(handler.documentClass);
    return handler;
  },
   ...
   */
  async Startup() {
    MathJax.startup.registerConstructor('labAdaptor', () => new labAdaptor(window));
    MathJax.startup.useAdaptor('labAdaptor', true);
    //
    //  Set the font and then load the output jax and packages
    //  (which may load font-specific extensions)
    //
    this.initFont();
    this.initRenderer();
    await MathJax.loader.load('output/chtml', ...texPackages);
    if (MathJax._.util.Locale) {
      await MathJax._.util.Locale.Locale.setLocale();
    }
    //
    ...
    //
    // Add our hooks into the menu for synchronizing the checkboxes,
    // load any needed extension, and wait for any menu promises
    //
    this.initMenu();
    //
    ...
    await Promise.all(this.doc.menu.constructor.loadingPromises.values());

    //
    // Update the font, if needed
    //
    if (MathJax.config.output.font !== fontName) {
      await this.setFont(MathJax.config.output.font);
    }
  ...
  initTeX() {
    ...
    for (const [key, value] of Object.entries(this.texinput)) {
      if (key === 'allowTexHTML') continue;             // handled later
  ...
  initOutput() {
    ...
    for (const [key, value] of Object.entries(this.output)) {
      if (key === 'breakInline') {
        output.linebreaks = {inline: value};
        continue;
      }
      if (key === 'font') {
        output.font = 'mathjax-' + value;
        continue;
      }
      if (key !== 'fontCache') {
        chtml[key] = value;
  ...
  /**
   * Clear the renderer menu setting (it will be set from the kept options)
   */
  initRenderer() {
    MathJax.config.startup.output = this.render.jax.toLowerCase();
    const {Menu} = MathJax._.ui.menu.Menu;
    const data = JSON.parse(localStorage.getItem(Menu.MENU_STORAGE));
    if (!data?.renderer) return;
    delete data.renderer;
    localStorage.setItem(Menu.MENU_STORAGE, JSON.stringify(data));
  },
  ...
  initFont() {
    if (window.location.search === '')  return;
    this.Load(); // sets this.output.font
    MathJax.config.output.font = 'mathjax-' + Lab.output.font;
  },
